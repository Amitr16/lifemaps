...
export const useLifeSheetStore = create(
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      // Work assets setters for live chart updates
      setWorkAssets: (updates) => set((state) => {
        state.workAssets = { ...state.workAssets, ...updates };
        get().recalculateAll();
      }),

      setWorkAssetStreams: (streams) => set((state) => {
        state.workAssets.extraStreams = Array.isArray(streams) ? streams.map(s => ({ ...s })) : [];
        get().recalculateAll();
      }),
// Calculation engine
